...
(function(){
  ...
  ITEMS.forEach(function(it){
    ...
    btns[0].addEventListener('click',function(){qty[it.id]=Math.max(0,qty[it.id]-1);byId('q-'+it.id).textContent=qty[it.id];calc();});
    btns[1].addEventListener('click',function(){qty[it.id]++;byId('q-'+it.id).textContent=qty[it.id];calc();});
  ...
  ['p-prop','p-stat','p-total','p-dots','p-e','p-budget','p-perk','p-cd'].forEach(function(id){
    byId(id).addEventListener('input',calc); byId(id).addEventListener('change',calc);
  });
  ...
  function calc(){
    ...
    var st=STAT[byId('p-stat').value];
    var S=Math.max(0,parseFloat(byId('p-total').value)||0);
    var dots=Math.max(0.1,parseFloat(byId('p-dots').value)||0.1);
    var E=parseFloat(byId('p-e').value)||10;
    var budget=Math.max(0,parseFloat(byId('p-budget').value)||0);
    var perkMult=1+(Math.max(0,parseFloat(byId('p-perk').value)||0)/100);
    var cdRed=parseFloat(byId('p-cd').value)||1;
    ...
    byId('r-gain-sub').textContent=itemUnits+' item'+(itemUnits===1?'':'s')+' eaten';
    ...
    if(anyPrice && gain>0){
      byId('r-cost').textContent='$'+nf.format(Math.round(cost/gain));
      byId('r-cost-sub').textContent='$'+nf.format(Math.round(cost))+' total';
    }else{
      byId('r-cost').textContent='—'; byId('r-cost-sub').textContent='enter prices';
    }
  }
...
<header class="hero"><div class="wrap">
  ...
  <p class="lede">Turn energy into stats fast, and spend cash so it actually pays off. Happy jumps, builds, schedules, items and the numbers behind them. New to Torn? <a href="{base}/start/">Start here</a> first.</p>
...
<section id="new"><div class="wrap">
  ...
  <ol class="steps" style="margin-top:22px">
    <li><div><b>Move into better housing, then do the New Player Missions.</b><small>They teach the systems and pay cash, credits and energy (500 in one step), and that energy trains better at higher happy. Take the one-time piggy bank offer; save mission credits for books and weapon upgrades. <a href="{base}/start/#missions">More →</a></small></div></li>
...
<section id="planner"><div class="wrap">
  ...
  <div class="plan-grid" style="margin-top:22px">
    ...
    <div class="card">
      ...
      <div class="field" style="margin-bottom:4px"><span class="flabel" id="lbl-mults">Candy happy multipliers</span>
    ...
    <div class="card">
      ...
      <div class="field" style="margin-top:14px;margin-bottom:0">
        <span class="flabel" id="lbl-xtc">Multiplier (applied to current happy)</span>
        <div class="chips" role="group" aria-labelledby="lbl-xtc">
